...
const STARTER_SUGGESTIONS = [
  { prompt: "Who should I reach out to this week?", label: "Who to reach out to" },
  { prompt: "Draft a re-engagement message for Ryan Patel", label: "Draft message for Ryan Patel" },
  { prompt: "What's the status on my urgent files?", label: "Urgent file status" },
  { prompt: "Show me all overdue contacts", label: "Overdue contacts" },
];

const ThreadWelcome: FC = () => {
  return (
    <div className="aui-thread-welcome-root mb-6 flex flex-col items-center px-4 text-center">
      <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600">
      ...
      <p className="mt-1 text-xs text-muted-foreground">Ask about contacts, draft outreach, or review files</p>
    </div>
  );
};
    ...
    <div className="flex w-full flex-col gap-2 px-4">
      {STARTER_SUGGESTIONS.map((s) => (
        <ThreadPrimitive.Suggestion
          ...
          method="replace"
          autoSend
          className="fade-in slide-in-from-bottom-2 animate-in fill-mode-both border-border/60 hover:bg-muted w-full cursor-pointer rounded-xl border bg-background px-4 py-2.5 text-left text-sm transition-colors duration-200"
        >
          {s.label}
        </ThreadPrimitive.Suggestion>
      ))}
